    ...
    def wrapper(*args, **kwargs):
        ...
        while True:
            try:
                ...
                logger.warning(f"Retry {num_retries}/{max_retries} for {func.__name__}: {e} (waiting {delay:.1f}s)")
...
class SplitBackend(VLMBackend):
    ...
    VISION_PROMPT_TITLE = (
        "Pokemon Emerald screen. Identify: screen type (title, name input, gender select, "
        "professor speech, new game/continue). If menu visible, list options and which is highlighted. "
        "If name input, report current name, cursor position on character grid, and special buttons. "
        "Transcribe any dialogue text exactly."
    )

    VISION_PROMPT_BATTLE = (
        "Pokemon Emerald battle screen. Report: battle phase (action select, move select, "
        "animation, damage text, faint, level up, switch prompt). "
        "List menu options with which is highlighted. "
        "HP bars: player and opponent fill level and color. "
        "Transcribe any text box content exactly."
    )

    VISION_PROMPT_OVERWORLD = (
        "Pokemon Emerald overworld. Describe: location type, player facing direction, "
        "nearby NPCs with position relative to player, doors/exits/paths visible, "
        "obstacles blocking movement. If dialogue box visible, transcribe text exactly."
    )

    VISION_PROMPT_DIALOGUE = (
        "Pokemon Emerald dialogue screen. Transcribe dialogue text exactly. "
        "Identify speaker. If choices are shown, list all options and which is highlighted by cursor. "
        "Note if continuation arrow is visible."
    )

    VISION_PROMPT_MENU = (
        "Pokemon Emerald menu screen. Identify menu type. "
        "List all visible options/items in order, mark which has the cursor with [>]. "
        "Transcribe any text or descriptions shown."
    )
